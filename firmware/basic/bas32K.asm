;==================================================================================
; Modifications by Eric Hazen for the Retro-25 project 
;    (https://github.com/eshazen/retro-25)
; See <ESH> tags for my modifications
; Inserted my serial code at the end to keep it all in one file
;
;
;
;
...
WRKSPC  .EQU    0B045H             ; BASIC Work space
...
;        .ORG    8100H		; <ESH> lives in RAM
        .ORG    9100H		; above UMON <FIXME>
;;; NOTE: also need to change WRKSPC somewhere safe
...
;;; <ESH> edit a bit to check for break, then read character
;TSTBRK: RST     18H             ; Check input status
;        RET     Z               ; No key, go back
;        RST     10H             ; Get the key into A

TSTBRK:	call	umon_chkbrk	;<ESH> check for break, then input char
        RET     Z               ; No key, go back
        CP      ESC             ; Escape key?
        JR      Z,BRK           ; Yes, break
        CP      CTRLC           ; <Ctrl-C>
...
;STALL:  RST     10H             ; Wait for key <FIXME>
STALL:  call	umon_getc	;<ESH> read serial
...
;GETINP: RST	10H             ;input a character <FIXME>
GETINP: call	umon_getc	;<ESH> get serial input
        RET
...
;;; 
;;; serial port
;;; umon_putc:  send a character from A
;;; umon_getc:  receive a character to A
;;;
;;; 
...
;;;
;;; check for serial activity (break)
;;; if so, wait for end of activity, then receive a character
;;; return 0 (Z) if no character, else char in A
;;;
umon_chkbrk:
	in	a,(serial_port)
	and	data_bit
	jr	nz,nobrk
	;; got break, wait for it to end
isbrk:	in	a,(serial_port)
	and	data_bit
	jr	z, isbrk	;loop while low

	jr	umon_getc	;now go get a character

nobrk:	xor	a		; zero a, set Z
	ret
